...
开发环境与 CI 统一使用 Ruff 0.12.0。当前 `pytest --collect-only -q` 收集 **498** 项测试；新增或删除用例后应以该命令输出为准。数据层定向验证可运行：
