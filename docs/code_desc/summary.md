...
| 测试与静态检查 | pytest 9.0.3（106 文件 / 1184 个 `test_*` 函数，CI `-n auto --timeout=60`）+ Ruff 0.12.0（`F` / `T201` / `I` / `B905`） |
...
自基线 `6cbe8b6`（2026-09-07）以来，本轮校准覆盖了以下变更：

**新增能力**
- **卡牌百科变更捕获**（624c8c5）：`CardSyncService`（手动检查官网手牌库与本地卡牌差异）、`card_sync_store`（CardSnapshot 覆盖式快照 + CardChangeRecord 追加式变更记录）、`card_baike`（官网手牌库抓取清洗与逐卡 diff 基元）、`CardSyncDialog`（检查与同步确认对话框）、`collect_stale_card_curated()`（卡牌精化时效检查）
- **实战配队逻辑删除**（0d7c73e / 2229efc）：`Combo` 新增 `deleted` / `deleted_at` 字段；`ComboManager` 新增 `delete_combo()` / `restore_combo()` / `list_all_combos()`；`list_combos()` 过滤 deleted；`get_combo()` 包含 deleted（供编辑覆盖检查与导入合并）；导入合并时 deleted 记录无条件保留并屏蔽同 key 源记录
- **势力配色升级**（1398692）：`config/faction_colors.json` 从 dict 升级为数组结构（`[{faction, color}, ...]`，数组位置即筛选界面展示顺序）；`faction_colors.py` 新增 `sort_factions_by_config()`
- **精化时效复核**（490dda9）：`audit_service` 新增 `collect_stale_curated()`（技能块精化时效，curated.updated_at 对比变更时间轴）与 `collect_stale_card_curated()`（卡牌块精化时效）；新增 AuditIssue kind：`curated_stale`（技能级确证）、`curated_stale_possible`（武将级存疑）、`card_curated_stale`（卡牌）

**修正的失效描述**
- **TRIGGER_OVERRIDES 拆除**（7392e7b）：`hero_timeline.py` 删除 TRIGGER_OVERRIDES 映射表、TRIGGER_OVERRIDES_AUTHORED 常量、stale_overrides() 函数，回归纯时间轴读写与版本戳职责
- 代码规模台账：106 文件 / 1184 个 `test_*` 函数（原 100 文件 / 1129 个）
