...
├── match_guide_panel.py        # 对局攻略页面（四个空白扩展板块）
│
...
`MatchGuidePanel` 与武将浏览、选将推荐处于同一主窗口 Tab 层级。首期只建立四个独立的空白容器，不绑定具体业务语义；`update_block(index, data)` 和 `clear_blocks()` 为后续可视化内容预留入口。
