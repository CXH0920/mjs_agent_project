...
追加内容仅由 `CardAnnotationEditDialog` 保存：版本效果记录追加到对话框后会重建记录列表，底部“保存”才写入文件，并阻止同字段生效中时间区间重叠；保存失败时对话框保持草稿。当前日期落在生效区间内的 `active` 记录会标记为“当前生效”、置顶并使用浅蓝强调，其余版本按状态和生效日期排序。`CardFieldSchemaDialog` 支持新增、修改显示属性、停用和归档字段，已有值的字段禁止改类型。归档字段及已失去定义的历史字段在详情页只读保留。卡牌切换时，详情区会递归释放控件和嵌套布局，避免旧卡片的操作栏残留。
