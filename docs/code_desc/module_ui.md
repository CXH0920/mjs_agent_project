...
所有多选对话框使用独立的已选 ID 集合，因此搜索或切换势力只会刷新可见列表，不会丢失已勾选的武将。多选界面提供“全选当前筛选”“清空已选”、可删除的已选标签和随选择数量变化的任务确认按钮；`GuideFetchDialog` 显示“生成 N 篇攻略”。

`SynergyPairDialog` 允许选择 2~8 个武将，并实时显示总组合数、已有相性数和本次待生成数。默认跳过已有相性，用户可选“重新生成并覆盖”；工作流将该选择传递给相性服务，服务以 CLI `--update` 参数执行覆盖生成。

四类选择界面统一使用 `CheckableComboBox` 作为势力筛选控件：输入区显示彩色可删除标签，超过 5 个势力显示剩余数量；右侧按钮用向下/向上箭头表示筛选弹窗的收起/展开状态；展开后提供势力搜索、浅蓝色复选列表、全选、反选和确定操作。
...
- 点击“阅读完整攻略”打开 `GuideDetailDialog`；其中的正文预览明确标注双击方式，双击后由 `GuideMarkdownDialog` 阅读完整 Markdown 正文。
...
RecommendationPanel (QWidget)
 ...
 └── QScrollArea → QGridLayout (4行 × 2列，小尺寸时滚动)
      └── HeroCardWidget × 8
           ...
           └── 信息区
               ├── 定位 · 推荐指数（例如 `辅助 · 推荐指数：92 / S`；悬停或点击指数查看明细，右侧圆形感叹号悬停查看口径）+ [技能] [攻略] 按钮
               ├── 最佳搭档优先展示，其余高相性组合（OCR 模式下仅显示当前 8 人相性）
               └── 胜率 + 异常数据状态 + 前三 🥇🥈🥉 奖牌
       └── HeroSkillDialog（来自 ui/shared/hero_dialogs.py，按技能名称分 Tab 展示描述和结算）
```

`recommendation_panel.py` 只保留推荐数据更新、相性加载、OCR 导入、手动重建推荐指数和截图信号协调；初始空状态直接提供当前模拟器识别和本地图片导入入口。胜率和推荐指数快照由 `RecommendationService` 一次读取，前三胜率排名基于数值快照计算，不再从卡片文本反解析。卡片按“定位 · 推荐指数、最佳搭档、胜率与异常数据状态、技能/攻略操作”呈现；指数有效时不显示状态，只有“指数待更新”“数据不足”和“OCR 待确认”等异常状态占用空间。官方榜单导入后，页面显示“推荐指数待重建”提示及“立即重建”；日常“保存截图”和“重建推荐指数”位于“更多”菜单，重建能力始终可再次手动执行。用户确认三份榜单后重建并覆盖 `武将推荐指数.csv`，同时清除待重建状态。OCR 导入和轮询仅读取已有快照。`style.py` 提供主色、成功、警告、弱化文本、表面和边框等语义色 token，推荐页与对局页的固定样式优先复用，势力色和前三名徽章仍按业务数据动态计算。可独立维护的展示组件已拆分为：
...
主级“资料库”内容页以“资料库”标题建立页面归属，再用浅色下划线式二级切换器提供“武将资料”和“卡牌图鉴”。`CardManagementPanel` 不再重复显示面包屑标题，使用左侧列表、右侧详情布局，只读展示官方卡牌名称、类型、牌堆数量、简述和规则详解；基础区固定显示锁定说明。简述与规则详解会将连续换行归并为单个换行，并随详情宽度自动换行。搜索覆盖基础字段和追加内容，支持类型及加强/削弱、当前生效、待核实状态筛选。左侧类型标题为不可选的浅色背景加粗分组行，刷新后自动选中首张卡牌；维护者使用右上角“更多”菜单进入追加字段管理。

追加内容仅由 `CardAnnotationEditDialog` 保存：版本效果记录可通过“新增一条版本记录”加入列表，底部“保存”也会一并收集尚未点击新增按钮的已填写版本表单，并阻止同字段生效中时间区间重叠；版本或效果说明未填时显示对应字段的中文提示，保存失败时对话框保持草稿。当前日期落在生效区间内的 `active` 记录会标记为“当前生效”、置顶并使用浅蓝强调，其余版本按状态和生效日期排序。`CardFieldSchemaDialog` 支持新增、修改显示属性、停用和归档字段，已有值的字段禁止改类型。归档字段及已失去定义的历史字段在详情页只读保留。卡牌切换时，详情区会递归释放控件和嵌套布局，避免旧卡片的操作栏残留。
