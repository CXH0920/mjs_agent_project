...
PaddleOCR 多路证据 → OCR 文字 + 置信度
  ...
  ├── 全部证据族 ≥ 0.995 一致读出同一词表外原文 → 抑制评分决胜，不绑定任何词表候选
  │    ├── 存在编辑距离候选 → unresolved，保留候选待人工确认
  │    └── 完全无候选 → unknown_new_hero，保留原文
...
词表外新武将保护：官方上新武将而百科数据滞后时，其名称会落在多个词表候选的编辑距离闭包内，若按候选内评分决胜会被静默绑到形近的既有武将（实测"王导"以 0.999 置信度被绑成"王异"）。判据要求全部证据族（≥2 族）以 ≥0.995 的置信度一致读出同一词表外原文——三个不同预处理视图一致才可能是图上真实文字，单次高置信度读数可能"自信地错"。但高置信度一致同样出现在生僻字误读场景（PaddleOCR 词表缺生僻字时会被整字丢弃或稳定替换，实测"王濬"只读出"王"、"王翦"可能读成"王浚"），无法与新武将区分，因此有候选时必须保留候选走人工确认，只有完全无候选的读数才判定为 `unknown_new_hero`（如三字新武将落在所有编辑距离闭包之外的情形）。该检查在确定性纠错（exact / unique_prefix / unique_similarity，含 `SAFE_SUBSTITUTION_WHITELIST` 混淆字对）之后执行，已知高频误读（如"王翡"→王翦）不受影响；置信度不足 0.995 的一致读数仍走候选内评分。历史上有过单次读数版保护（`_HIGH_CONFIDENCE = 0.995`，b6f1452 引入），在证据体系迁移（bbc0ede）中未跟迁、随死代码清理（a714f0f）遗失，本规则是其在多路证据语义上的重建。
...
### 官方榜单单元格 OCR 必须使用完整检测+识别流程（2026-08-14 补充）

- 单元格识别调用必须为完整 `det + rec` 流程（`ocr(candidate, cls=False)`），禁止以性能为由传入 `det=False` 跳过检测网络。
- **为什么：** `det=False` 时识别网络直接处理整格输入，会把单元格边缘、卡框边线与残留笔画读成固定模式幻觉字符（前缀 `東/廣/時/電/適/嬰`，后缀 `鍵/縮/來/廠/膚/銀`，且输出繁体字形），使名称与词表编辑距离 ≥2，超出名称校正的 1 次编辑阈值，最终触发名称校验失败并拒绝导入。2026-08-04 曾以“省 det 固定开销”引入 `det=False`，2026-08-14 用 20260715 批次实测确认其为幻觉来源，已恢复完整流程。
- **验收：** 任一官方榜单批次导入时名称校验必须全部通过；回归测试须覆盖 20260715 批次中曾失败的 `東曹仁鍵`、`廣信陵君膚`、`東吕布銀` 等行；若未来再次评估跳过检测，须先完成“整格裁剪去边缘 + 全批次名称校验通过”的证明。
