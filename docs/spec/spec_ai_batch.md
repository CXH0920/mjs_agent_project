...
`PlaywrightGenerator` 只处理提示词、上下文标记、JSON 提取和模型校验；`DeepSeekBrowserSession` 独占 Playwright、Edge、页面选择器、登录等待和回复读取。生成器通过 `send_and_wait(prompt)` 使用页面会话，并在 `close()` 时统一关闭。

**为什么：** 页面结构和浏览器生命周期变化频繁，生成数据契约则相对稳定。分离后可以用假页面独立验证发送、等待和关闭逻辑，也能在不启动浏览器的情况下验证攻略与相性生成。
...
AI 批量生成负责补充单将攻略和两两相性数据，不负责实时对局判断、胜率预测或自动修改基础武将资料。生成结果必须先经 JSON 提取、字段规范化和 Pydantic 校验，再按批次原子写入；任何 AI 文本都不能绕过数据模型直接进入 UI。

### 6.2 失败语义

- 单项生成失败保留该项旧记录，其他成功项继续提交；全任务结果以 CLI 退出码报告。
- 浏览器模式无法提供 token usage 时返回 `None`，调用方不得将其视为生成失败。
- 取消只终止当前子进程；父进程由 finished 信号统一收尾，不在 GUI 线程同步等待。
- 相性任务的输入武将必须存在于本地武将库，且不同 ID 的无向配对只生成一次。
- 武将文件必须通过 `HeroManager` 的根类型、逐条模型和重复 ID 校验；任一错误都终止生成，不允许只使用部分有效记录继续执行。
- 攻略或相性断点文件存在错误时，原文件先改名为同目录的 `.corrupt-时间戳.json`，再将 Manager 已验证的记录原子写回；备份失败必须中止，禁止覆盖原文件。

### 6.3 验收

- API 与浏览器模式对同一有效输入均产生可由对应 Pydantic 模型校验的结果。
- 含代码块、分隔线、字符串字面换行、字符串 ID 的模型返回能按既定降级链正确处理或明确失败。
- 2～8 名武将的相性任务总数严格等于 C(N,2)，每条 stdout 进度均可被 UI 解析。
- 一批中出现失败项时，已成功项落盘、失败项旧数据不变，任务以非零退出码结束并保留可读错误信息。
- 非法武将输入不会进入生成器；损坏断点文件可从 `.corrupt-*` 备份恢复，当前文件只包含通过模型校验的记录。
