...
日常入口名称使用“卡牌图鉴”，而不是“卡牌管理”，突出查阅规则和版本信息的主要用途。面向维护者的“管理追加字段”收纳在右上角“更多”菜单中，不干扰常规浏览用户。页面使用“左侧列表 + 右侧详情”布局。
...
- 同一追加字段的版本记录中，日期范围覆盖当天且状态为 `active` 的记录为“当前生效”：必须显示“当前生效”文字、置顶，并以浅蓝底与左侧强调线突出；其他记录按状态和生效开始日期排列。
...
- 点击“追加版本记录”后，新记录立即留在当前编辑对话框的记录列表中；点击底部“保存”后才写入文件，且不得覆盖旧版本记录。
