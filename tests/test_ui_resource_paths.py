...
from src.config.env import IMAGES_DIR, IMAGES_OUTPUT_DIR, PROJECT_ROOT, SCREENSHOTS_DIR
...
def test_ui_resources_resolve_from_project_root() -> None:
    ...
    assert IMAGES_OUTPUT_DIR == PROJECT_ROOT / "images"  # crawler 下载写盘目录，必须是可写根
    assert CRAWLER_IMAGES_DIR == IMAGES_OUTPUT_DIR  # crawler 接入点锁定
